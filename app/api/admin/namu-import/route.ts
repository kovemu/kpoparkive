...
export async function POST(request: Request) {
  ...
  try {
    ...
    const results: { title: string; depth: number; status: string; links: number; images: number }[] = [];
    let errorCount = 0;
    let skippedCount = 0;
    ...
    while (queue.length && seen.size < maxDocuments) {
      ...
      if (!shouldCrawlTitle(current.title, rootTitle, includePrefixes, includeTitles)) {
        skippedCount += 1;
        continue;
      }
      ...
      try {
        ...
        results.push({ title: current.title, depth: current.depth, status: "fetched", links: snapshot.links.length, images: snapshot.images.length });

        if (current.depth < maxDepth) {
          for (const link of snapshot.links) {
            if (!seen.has(link) && shouldCrawlTitle(link, rootTitle, includePrefixes, includeTitles)) {
              queue.push({ title: link, depth: current.depth + 1 });
            }
          }
        }
      } catch (error) {
        errorCount += 1;
        results.push({ title: current.title, depth: current.depth, status: error instanceof Error ? error.message : "error", links: 0, images: 0 });
      }
    }
    ...
    await db(`import_runs?id=eq.${runId}`, {
      ...
      body: JSON.stringify({
        ...
        result: { documents: results, includePrefixes, includeTitles },
