...
function queueRows(documentId: string, rootTitle: string, sourceTitle: string, blocks: ParsedBlock[]) {
  return blocks.flatMap((block) => {
    if (block.type === "image") return [{ source_document_id: documentId, root_title: rootTitle, source_title: sourceTitle, asset_type: "image", source_ref: block.source_ref, label: block.alt || null, provider: block.url ? "direct" : "namu_file", role: block.role || null, metadata: { url: block.url || null } }];
    if (block.type === "video") return [{ source_document_id: documentId, root_title: rootTitle, source_title: sourceTitle, asset_type: "video", source_ref: block.url, label: block.label || null, provider: block.provider, metadata: { video_id: block.video_id || null } }];
    if (block.type === "external-link") return [{ source_document_id: documentId, root_title: rootTitle, source_title: sourceTitle, asset_type: "external_link", source_ref: block.url, label: block.label, provider: (() => { try { return new URL(block.url).hostname; } catch { return "external"; } })(), metadata: {} }];
    if (block.type === "internal-link") return [{ source_document_id: documentId, root_title: rootTitle, source_title: sourceTitle, asset_type: "internal_link", source_ref: block.target, label: block.label, provider: "namu", metadata: {} }];
    return [];
  });
}
...
export async function POST(request: Request) {
  ...
  try {
      ...
      try {
        ...
        if (assets.length) {
          await db("source_asset_queue?on_conflict=source_document_id,asset_type,source_ref", {
            method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=minimal" }, body: JSON.stringify(assets),
          });
        }
    ...
    return NextResponse.json({
      ok: true, rootTitle, parseVersion: PARSE_VERSION, documents: results.length,
      ...
      totalQueuedAssets: results.reduce((sum, r) => sum + r.queuedAssets, 0), results,
    });
  } catch (error) {
