...
import { parseWikiTemplates, replaceWikiTemplate } from "../../../lib/wikiTemplateEdit";
...
function parseHeadingModels(source: string) {
  let sectionIndex = 0;
  const headings: Array<{ key: string; sectionIndex: number; level: number; headingRaw: string }> = [];
  for (const line of source.replace(/\r\n?/g, "\n").split("\n")) {
    ...
    headings.push({
      ...
      headingRaw: match[2].trim(),
    });
  }
  return headings;
}

function replaceHeading(source: string, change: HeadingChange) {
  const sectionKey = String(change.sectionKey || "").trim();
  const wanted = sectionIndexFromKey(sectionKey);
  if (wanted <= 0) throw Object.assign(new Error("Invalid heading section"), { status: 400 });

  const proposedWikitext = normalizeWiki(String(change.proposedWikitext || ""));
  if (!proposedWikitext || proposedWikitext.length > 500 || proposedWikitext.includes("\n")) {
    throw Object.assign(new Error("Section titles must be a single line between 1 and 500 characters."), { status: 400 });
  }

  const lines = source.replace(/\r\n?/g, "\n").split("\n");
  let index = 0;
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    const match = lines[lineIndex].match(HEADING_RE);
    if (!match) continue;
    index += 1;
    if (index !== wanted) continue;

    const oldRaw = match[2].trim();
    const oldLevel = match[1].length;
    const requestedLevel = Number(change.level || oldLevel);
    const level = Number.isFinite(requestedLevel) ? Math.max(2, Math.min(6, Math.round(requestedLevel))) : oldLevel;
    if (normalizeWiki(oldRaw) === proposedWikitext && oldLevel === level) {
      return { source, changed: false, oldRaw, newRaw: proposedWikitext, oldLevel, level };
    }
    const marks = "=".repeat(level);
    lines[lineIndex] = `${marks} ${proposedWikitext} ${marks}`;
    return { source: lines.join("\n"), changed: true, oldRaw, newRaw: proposedWikitext, oldLevel, level };
  }

  throw Object.assign(new Error("The section title no longer exists. Reload and try again."), { status: 409 });
}
...
export async function GET(request: Request) {
  try {
    ...
    return json({
      ...
      headings: parseHeadingModels(source),
      sections,
      tables,
...
export async function POST(request: Request) {
  try {
    ...
    for (const headingChange of body.headingChanges || []) {
      const result = replaceHeading(proposed, headingChange);
      if (!result.changed) continue;
      proposed = result.source;
      changeLabels.push(`Heading: ${result.oldRaw} → ${result.newRaw}`);
    }
