...
const workerId =
  valueArg("worker-id") ||
  "source-" + process.pid + "-" + Date.now().toString(36);

if (!runId) {
  console.error(
    "Usage: npm run namu:source-worker -- --run=<pipeline-run-id> [--once]",
  );
  process.exit(2);
}
requireServiceRole();
...
function runNodeScript(script, scriptArgs = [], acceptableCodes = [0]) {
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", (code, signal) => {
      ...
      if (acceptableCodes.includes(numericCode)) {
        resolve(numericCode);
      } else {
        reject(
          new Error(
...
function runSourceRenderer(title) {
  return new Promise((resolve, reject) => {
    const child = spawn(
      ...
      [
        "--no-node-snapshot",
        path.resolve(ROOT, "scripts/namumark-thetree-compat-poc.mjs"),
        title,
      ],
      {
        cwd: ROOT,
...
async function repairFromCapturedArtifacts(doc) {
  ...
  const missingFiles = Array.isArray(meta?.missingFiles)
    ? meta.missingFiles
    : [];

  if (missingFiles.length > 0) {
    console.log(
      "SOURCE REPAIR from captured DOM · files=" +
        missingFiles.length +
        " · " +
    ...
    await runNodeScript(
      ...
      console.warn("CAPTURED VIDEO RECOVERY WARN " + String(error?.message || error));
    });
  ...
  const missingTemplates = Array.isArray(meta?.missingTemplates)
    ? meta.missingTemplates
    : [];

  for (const templateTitle of missingTemplates.slice(0, 30)) {
    console.log(
      "SOURCE REPAIR from captured DOM · template=" +
        templateTitle +
        " · owner=" +
        current.source_title,
    );
    ...
    await runNodeScript(
      ...
      console.warn(
        "CAPTURED TEMPLATE RECOVERY WARN " +
...
function validateSource(doc) {
  ...
  if (compat !== "modern-namu-compat-v8") {
    blockers.push("source_stale_compat:" + (compat || "none"));
  }
  if (patchset !== "modern-namu-v2") {
...
console.log(
  "Kpoparkive Source Worker · captured-data-only · " +
    workerId +
    " · run=" +
    runId,
);
...
while (true) {
  ...
  try {
    ...
    if (
      ...
      after = await repairFromCapturedArtifacts(after);
    ...
    if (blockers.length) {
      throw new Error(
        "source_qa_failed:" +
          blockers.join("|") +
          "|manual_collection_may_be_incomplete",
      );
    }
    ...
    await updatePipelineJob(job.id, {
      ...
      checkpoint: {
        qa: "pass",
        collectionMode: "manual-extension",
