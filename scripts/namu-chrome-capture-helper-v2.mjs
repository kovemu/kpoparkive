...
async function upsertCapture(meta, storagePath, contentType, width, height, byteLength, captureStatus, matchedRows) {
  const body = {
    root_title: meta.rootTitle,
    source_title: meta.sourceTitle,
    file_name: meta.fileName,
    canonical_key: canonicalFileKey(meta.fileName),
...
async function patchResolved(rows, stored, meta, contentType, width, height, byteLength) {
...
const stats = { received: 0, resolved: 0, noQueue: 0, staged: 0, errors: 0 };
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/health") {
    json(res, 200, { ok: true, service: "kpoparkive-namu-chrome-capture-helper-v2", port: PORT, supabase: SUPABASE_URL, stats });
    return;
  }
  ...
  try {
    ...
    const fileKey = canonicalFileKey(meta.fileName);
    ...
    await patchResolved(rows, stored, meta, contentType, width, height, bytes.length);
    await upsertCapture(meta, stored.storagePath, contentType, width, height, bytes.length, "matched", rows.length);
    stats.resolved += 1;
    console.log(`RESOLVED ${meta.fileName} -> ${width}x${height}, ${(bytes.length / 1024).toFixed(1)} KB (${rows.length} queue rows)`);
    console.log(`  ${stored.storagePath}`);
    json(res, 200, { ok: true, status: "resolved", fileName: meta.fileName, matchedRows: rows.length, storagePath: stored.storagePath, bytes: bytes.length, width, height, queueRowsVisible: cache.rows.length });
  } catch (error) {
    stats.errors += 1;
...
server.listen(PORT, HOST, () => {
  console.log("Kpoparkive Namu Chrome capture helper v2");
